...
class PagesController < ApplicationController
  ...
  def index
    ...
    metrics = [method(:num_meals_available), method(:num_schools_joined), method(:num_users_signedup),
               method(:num_meals)]
  ...
  def num_users_signedup
    ...
    all_users.each do |user|
      if user_type_index.zero? && user.user_type == 'donor'
        user_count += 1
  ...
  def num_meals
    ...
    transaction_type_array = %w[donated received]
    ...
    when 3
      ...
      all_transactions.each do |transaction|
        res += 1 if transaction_type == transaction.transaction_type && today.year == transaction.created_at.year
      end
    end
